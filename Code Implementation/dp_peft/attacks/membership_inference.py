...
class MembershipInferenceAttack:
    ...
    def compute_loss_per_sample(self, dataloader: DataLoader) -> np.ndarray:
        self.model.eval()
        losses = []
        with torch.no_grad():
    ...
    def likelihood_ratio_attack(
        ...
        labels = np.concatenate([np.ones(len(train_ratio)), np.zeros(len(test_ratio))])
        scores = np.concatenate([-train_ratio, -test_ratio])
    ...
    def run_attack(
        ...
        results = {}
        results['threshold_attack'] = self.loss_threshold_attack(train_loader, test_loader)
        if shadow_model is not None:
            results['likelihood_ratio_attack'] = self.likelihood_ratio_attack(
                train_loader, test_loader, shadow_model
            )
        return results
