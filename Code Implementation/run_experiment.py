...
def main():
    ...
    config = {
        'model': args.model, 'dataset': args.dataset, 'placement': args.placement,
        'epsilon': args.epsilon, 'delta': args.delta, 'peft_method': args.peft_method,
        'seed': args.seed, 'epochs': args.epochs, 'batch_size': args.batch_size,
        'learning_rate': args.lr, 'device': args.device
    }
    config.update(get_environment_info())
    setup_logging(project_name=args.wandb_project, run_name=run_name, config=config)
    ...
    if args.model in ['bert', 'distilbert']:
        ...
        train_loader, test_loader = get_text_dataloaders(
            dataset_name=args.dataset, tokenizer_name=model_name,
            batch_size=args.batch_size, max_length=128
        )
    else:
        model = get_vision_model(
            model_name='google/vit-base-patch16-224', num_labels=10, adapter_hidden_dim=64
        )
        train_loader, test_loader = get_vision_dataloaders(
            dataset_name=args.dataset, batch_size=args.batch_size, image_size=224
        )

    dp_placement = get_placement_strategy(model=model, strategy_name=args.placement, max_grad_norm=1.0)
    ...
    trainer = DPPEFTTrainer(
        ...
        optimizer=optimizer, dp_placement=dp_placement, device=args.device,
        target_epsilon=args.epsilon, target_delta=args.delta,
        epochs=args.epochs, checkpoint_dir=args.checkpoint_dir, results_dir=args.results_dir
    )
    ...
    print(f"\nDone. Accuracy: {results['final_accuracy']:.4f}, Epsilon: {results['final_epsilon']:.2f}")
