#!/usr/bin/env python3
"""
Fixed DP-PEFT Experiment with Proper Hyperparameters
Addresses the low accuracy issue by using appropriate LR and clipping for DP-SGD
"""
...
def run_experiment(
    ...
    delta=1e-5,
    max_samples=20000,  # INCREASED for better results
    device='cuda'
):
    """Run a single experiment with specified placement."""
    set_seed(42)
    
    # FIXED: Different learning rates for DP vs no-DP
    if placement == 'no_dp':
        lr = 2e-5  # Standard fine-tuning LR
    else:
        lr = 5e-4  # MUCH HIGHER for DP-SGD (25x increase)
    
    print(f"\n{'='*70}")
    print(f"Running: {placement} | ε={target_epsilon} | epochs={epochs} | lr={lr}")
    print(f"{'='*70}")
...
def main():
    ...
    print("="*70)
    print("• Learning rate for DP: 5e-4 (25x higher than before)")
    print("• Max gradient norm: 5.0 (5x higher clipping threshold)")
    print("• Training samples: 20,000 (2x more data)")
    print("• Epochs: 15 (50% more training)")
    print("="*70 + "\n")
    ...
    for placement in args.placements:
        try:
            ...
            with open(output_dir / f'fixed_bert_agnews_{placement}_eps{args.epsilon}.json', 'w') as f:
    ...
    with open(output_dir / f'fixed_experiment_{timestamp}.json', 'w') as f:
...
def generate_plots(all_results, output_dir, timestamp):
    ...
    axes[0, 1].axhline(y=8.0, color='red', linestyle='--', alpha=0.7, label='Target ε=8')
    axes[0, 1].legend()
    ...
    axes[1, 0].axhline(y=0.5, color='green', linestyle='--', alpha=0.7, label='Random')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f'fixed_bert_agnews_results_{timestamp}.png', dpi=150, bbox_inches='tight')
    plt.savefig(output_dir / f'fixed_bert_agnews_results_{timestamp}.pdf', bbox_inches='tight')
    plt.close()
